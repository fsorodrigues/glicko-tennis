...
def normalize_path(dir_path):
    return path.abspath(path.join(CURRENTPATH, dir_path))
...
def readATPMatches(dirname):
    ...
    allFiles = glob.glob(normalize_path(dirname + "/atp_matches_" + "????.csv"))
...
def readFMatches(dirname):
    ...
    allFiles = glob.glob(normalize_path(dirname + "/atp_matches_futures_" + "????.csv"))
    matches = pd.DataFrame()
    container = list()
    for filen in allFiles:
        df = pd.read_csv(filen,
...
#     allFiles = glob.glob(normalize_path(dirname + "/atp_rankings_" + "*.csv"))
